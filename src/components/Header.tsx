...
export default function Header() {
    ...
    return (
        <>
            <nav className="bg-white flex items-center justify-between px-4 py-1 box-border gap-4 sticky top-0  h-16 z-50 shadow-lg shadow-gray-200">
                <div>
                    <h1 className="text-xl font-bold text-rose-600">ProjectHub</h1>
                </div>
                <div className={`md:w-[70%] md:flex justify-between md:py-1 md:h-full items-center md:static md:flex-row overflow-hidden md:overflow-visible md:px-3 fixed flex-col h-[100vh] transition-all duration-500 delay-0 w-0  ${navActive ? "w-60" : ""
                            } top-0 right-0 bg-gray-100 md:bg-transparent shadow-gray-300 shadow-3xl md:gap-2 gap-1 py-2`}
                    >

                    <ul
                        className="md:w-[70%] flex flex-col justify-between md:flex-row">
